#!/usr/bin/env python3
"""Stage platform wheels for agenvo from prebuilt release binaries."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage agenvo PyPI wheels from release binaries.")
    parser.add_argument(
        "--release-version",
        required=True,
        help="Version to stage, for example 0.2.0.",
    )
    parser.add_argument(
        "--vendor-src",
        type=Path,
...
setup(
    name="{PACKAGE_NAME}",
    version="{version}",
    description="Temporary room-based conversation log for agents",
    long_description=long_description,
    long_description_content_type="text/markdown",
    license="Apache-2.0",
    python_requires=">=3.8",
    packages=["{IMPORT_NAME}", "{IMPORT_NAME}.bin"],
    package_data={{"{IMPORT_NAME}": ["bin/*"]}},
    include_package_data=True,
    url="https://github.com/Xuanwo/agenvo",
    project_urls={{
        "Homepage": "https://github.com/Xuanwo/agenvo",
        "Repository": "https://github.com/Xuanwo/agenvo",
        "Issues": "https://github.com/Xuanwo/agenvo/issues",
    }},
    entry_points={{"console_scripts": ["agenvo={IMPORT_NAME}._runner:main"]}},
...
def main() -> None:
    binary_name = "agenvo.exe" if os.name == "nt" else "agenvo"
    binary_path = Path(__file__).resolve().parent / "bin" / binary_name
    if not binary_path.exists():
        raise FileNotFoundError(f"agenvo binary not found: {binary_path}")
...
def prepare_stage_dir(
    *,
    ...
    (bin_root / "__init__.py").write_text("", encoding="utf-8")

    source_binary = vendor_src / target / "agenvo" / binary_name
...
def stage_wheel(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f"agenvo-pypi-stage-{target}-") as stage_dir_str:
