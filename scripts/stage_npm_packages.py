#!/usr/bin/env python3
"""Stage agenvo npm tarballs for release."""
...
_SPEC = importlib.util.spec_from_file_location("agenvo_build_npm_package", BUILD_SCRIPT)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Package name to stage. May be provided multiple times. Defaults to agenvo.",
    )
    parser.add_argument(
        "--vendor-src",
        type=Path,
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in PLATFORM_PACKAGES:
        platform = package.removeprefix("agenvo-")
        return f"agenvo-npm-{platform}-{version}.tgz"
    return f"agenvo-npm-{version}.tgz"
...
def main() -> int:
    args = parse_args()
    packages = args.packages or ["agenvo"]
